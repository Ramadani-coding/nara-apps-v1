...
function HomePage() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      try {
        ...
        if (!isMounted) return
        setError(loadError.message || 'Katalog premium belum bisa dimuat dari backend.')
      } finally {
        if (isMounted) {
          setIsLoading(false)
        }
  ...
  function handleInvoiceLookup() {
    ...
    navigate(`/order-status?invoice=${encodeURIComponent(normalizeInvoice(invoice))}`)
  }

  return (
    <div className="space-y-6 pb-10 md:space-y-7">
      <PhoneHero
        title="Akun premium digital tanpa ribet."
        subtitle="Checkout cepat, cek invoice instan, dan pilih layanan premium favoritmu langsung dari data backend."
      >
        <StatusBadge className="mt-1" tone="neutral">
          Nara Premium
        </StatusBadge>
      </PhoneHero>
